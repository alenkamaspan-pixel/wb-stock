...
def ozon_diagnostics_page():
    ...
    # ДЕВЯТАЯ ПРАВКА (20.09.2026, разовая точечная проверка): поставка
    # #128640668 (видна в личном кабинете Ozon — статус «Согласование
    # актов», создана 14.09) не появилась в списке выше даже после
    # расширения states до 1..11 (восьмая правка). Запрашиваем её статус
    # НАПРЯМУЮ по ID (Алёна прислала его со скриншота), в обход фильтра
    # states у /v3/supply-order/list — /v3/supply-order/get принимает
    # order_ids без всякой фильтрации по состоянию. Цель — узнать точное
    # числовое значение её статуса, чтобы включить его в states осознанно,
    # а не гадать дальше. Строка временная, уберём после того как разберёмся.
    _call(
        "ТОЧЕЧНАЯ ПРОВЕРКА: поставка #128640668 напрямую по ID (/v3/supply-order/get)",
        lambda: client.get_supply_orders_info([128640668]),
    )
    _call(
        "ТОЧЕЧНАЯ ПРОВЕРКА: поставка #128640668 — СЫРОЙ ответ, без разбора",
        lambda: client.get_supply_orders_info_raw([128640668]),
    )
