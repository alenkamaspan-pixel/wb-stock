...
class OzonClient:
    ...
    def list_supply_orders(self, states: list[str] | None = None, limit: int = 100) -> list:
        ...
        СЕДЬМАЯ ПРАВКА (20.09.2026, после ручной сверки Алёны с личным
        кабинетом Ozon): диапазон 1..9 молча пропускал реальные поставки —
        конкретно поставку #128640668 (статус в кабинете Ozon —
        «Согласование актов», 480 шт по КР-1029/CR-1230), которая создана
        14.09 и до сих пор не подтверждена актами. 1..9 не давал ошибки —
        то есть все значения 1..9 сами по себе валидны, но это не значит,
        что 9 — потолок enum'а: диапазон был подобран наугад ещё в ПЯТОЙ
        правке и никогда не проверялся на "а не пропускаем ли мы более
        высокие статусы". Судя по количеству разделов в личном кабинете
        Ozon (Новые / Подготовка к отгрузке / В пути и на приёмке /
        Согласование актов / Архив), реальных статусов явно больше 9.
        Расширила диапазон до 1..20 с запасом — если Ozon отклонит
        какое-то из новых значений, это будет видно прямо в ошибке
        /ozon-diagnostics (как и раньше), и диапазон можно будет сузить
        точно. Слишком широкий диапазон безопаснее слишком узкого: лишнее
        значение в худшем случае вызовет ошибку валидации (видно и
        чинится), а не тихо потерянную поставку (не видно вообще).

        ВОСЬМАЯ ПРАВКА (20.09.2026, сразу после деплоя седьмой): 1..20
        оказался слишком широким — live-ответ: "invalid
        SupplyOrderListRequest_Filter.States[11]: value must be one of
        the defined enum values". States[11] — 12-й элемент списка
        (индекс с нуля), то есть первое же отклонённое значение — 12.
        Ozon проверяет список по порядку и останавливается на первом
        недопустимом элементе, поэтому все значения до него (1..11)
        подтверждены как валидные самим Ozon. Верхняя граница enum'а —
        11. Сузила диапазон до 1..11 — теперь это не угадывание, а
        подтверждённый живым ответом Ozon полный список состояний."""
        limit = max(1, min(int(limit), 100))
        body: dict = {
            "limit": limit,
            "sort_by": 1,
            "filter": {"states": states if states else list(range(1, 12))},
        }
