...
def sync_fbs_once(client: OzonClient | None = None) -> dict:
    ...
    sync_once в sync.py) — уже известные (posting_number, line_no) строки
    с уже списанным остатком просто пропускаются.

    ОДИННАДЦАТАЯ ПРАВКА (19.09.2026, после первой реальной синхронизации
    на живых данных Алёны): если склад Ozon для отправления ещё не был
    привязан на странице «Склады» в момент первого обнаружения — остаток
    не списывался, писалось предупреждение, а строка всё равно сохранялась
    в ozon_postings. Раньше на этом всё — при каждом следующем запуске
    такая строка просто пропускалась НАВСЕГДА как «уже известная»,
    независимо от того, появился ли маппинг склада позже. Из-за этого,
    когда Алёна привязала оба ФБС-склада, уже увиденные до этого 30
    реальных отправлений остались несписанными без какой-либо ошибки —
    молча, как и остальные баги в этой интеграции, найденные через
    /ozon-diagnostics. Теперь уже известная, но ещё не списанная строка
    (stock_deducted=0) при каждом следующем запуске пробует сопоставить
    склад заново — и если он теперь находится, списывает остаток именно
    сейчас."""
        ...
        for raw in postings:
            ...
            for line_no, p in enumerate(products):
                existing = conn.execute(
                    "SELECT * FROM ozon_postings WHERE posting_number = ? AND line_no = ?",
                    (posting_number, line_no),
                ).fetchone()
                if existing:
                    if existing["stock_deducted"]:
                        continue
                    # ОДИННАДЦАТАЯ ПРАВКА: раньше это отправление уже было
                    # увидено (строка в ozon_postings есть), но тогда склад
                    # Ozon ещё не был сопоставлен ни с одним складом
                    # приложения — остаток НЕ списывался, только писалось
                    # предупреждение (см. ветку else ниже). Раньше на этом
                    # синхронизация НАВСЕГДА переставала возвращаться к
                    # такому отправлению — существующая строка просто
                    # пропускалась при каждом следующем запуске, даже после
                    # того как склад привязали на странице «Склады». Из-за
                    # этого 30 реальных продаж Ozon повисли несписанными,
                    # хотя маппинг склада уже появился. Теперь при каждом
                    # запуске для ещё не списанных строк склад ищется
                    # заново — и если он теперь нашёлся, остаток списывается
                    # именно сейчас, одним движением, а не задним числом
                    # прошлой датой (это не бэкфилл прошлого, а обычное
                    # обнаружение уже случившейся, но ранее пропущенной
                    # продажи, точно как только что появившееся отправление).
                    if warehouse:
                        _add_movement(
                            conn, existing["product_id"], warehouse["id"], MovementType.SALE,
                            -existing["quantity"],
                            comment=f"Отправление Ozon {posting_number} (склад сопоставлен позже исходной синхронизации)",
                        )
                        conn.execute(
                            "UPDATE ozon_postings SET stock_deducted = 1, warehouse_id = ?, updated_at = ? "
                            "WHERE id = ?",
